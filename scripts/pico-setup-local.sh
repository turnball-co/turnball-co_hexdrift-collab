#!/bin/bash
# README: this script is intended to assist the person executing the codebase to setup their local environment
#   it should be able to produce a functional local codebase for purposes of running the code, loading it onto 
#   devices such as your own locally connected PICO device or Zero Raspberry PIs.  You should also be able to 
#   use the resources with other compatible devices such as other Raspberry Pi instances and other similar 
#   hardware such as Arduinos and devices such as Orange Pis however those methods have not bee independantly
#   verified as I (ben@turnball.co) have only explicitly tested and found it operable for my own use cases 
#   which have been exlusively Raspberry Pi devices.
# Exit on error
set -e
